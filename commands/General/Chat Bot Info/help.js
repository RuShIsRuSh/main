	...
	async buildDisplay(message) {
		...
		for (const [category, list] of commands) {
			display.addPage(new MessageEmbed()
				...
				.setColor(color)
				.setDescription("*Note that nsfw commands in help menu will only be shown in nsfw chats.*\n*Same goes for permission commands, they'll only be shown for people who has the required permission to use them.*\n\n" + list.map(this.formatCommand.bind(this, message, prefix, true)).join('\n'))
