...
class Leaderboard extends Command {
  ...
  async run(msg, [page = 1]) {
    const list = msg.guild.members
      .filter((x) => x.settings.won > 0 && !x.user.bot)
      .array()
      .sort((a, b) => a.settings.won < b.settings.won ? 1 : -1);
    page -= 1;
    ...
    const embed = new MessageEmbed()
      .setColor(0xff0000)
      .setTitle(`=== ${process.env.botName} Global Leaderboard ===`)
    ...
    top.map((x, i) => {
      leaderboard.push(`${(page * 10 + (i + 1)).toString()}. ${x.user.username}: ${x.settings.won.toLocaleString()} ${this.client.constants.currency}`);
    });
